    ...
    <>
      ...
      {tela === 'identificacao' && (
        <IdentificacaoDigital
          ...
          onVeiculoEncontrado={(veiculo) => {
            ...
            setClienteAtual({ idCliente: veiculo.idCliente, nome: veiculo.nomeCliente, telefone: veiculo.telefoneCliente })
            setTela('checklist')
          }}
        />
      )}
      ...
      {tela === 'sintomas' && (
        <RegistroSintomas
          ...
          onSalvar={(os) => { setOsAtual(os); setTela('osSalva') }}
      ...
      {tela === 'osSalva' && (
        <OSSalva
          onNavigate={navegarMenu}
          onVoltarInicio={() => { setOsAtual(null); setVeiculoAtual(null); setClienteAtual(null); setTela('identificacao') }}
          osAtual={osAtual}
          veiculoAtual={veiculoAtual}
          clienteAtual={clienteAtual}
        />
      ...
      {tela === 'receberOS' && (
        <ReceberOSDigital
          onNavigate={navegarMenu}
          onAbrirOS={(os) => { setOsAtual(os); setTela('visualizarOS') }}
        />
      )}
      ...
      {tela === 'resumoOrcamento' && (
        <ResumoOrcamento
          ...
          onGerarOrcamento={(orc) => { setOrcamentoAtual(orc); setTela('enviarOrcamento') }}
          osAtual={osAtual}
        />
      )}
      ...
      {tela === 'decisaoCliente' && (
        <DecisaoCliente
          ...
          onAprovar={() => { setResultadoCliente('aprovado'); setTela('resultadoFinal') }}
          onNaoAprovar={() => { setResultadoCliente('naoAprovado'); setTela('resultadoFinal') }}
          osAtual={osAtual}
          orcamentoAtual={orcamentoAtual}
        />
      )}
      ...
      {tela === 'atribuicaoTecnico' && (
        <AtribuicaoTecnico
          ...
          onConfirmarTecnico={(osAtualizada) => { if (osAtualizada) setOsAtual(osAtualizada); setTela('execucaoTecnico') }}
          osAtual={osAtual}
        />
      )}
      ...
      {tela === 'acompanhamentoCliente' && (
        <AcompanhamentoCliente
          onVoltarInicio={() => { setOsAtual(null); setTela('identificacao') }}
          osAtual={osAtual}
        />
      )}
